...
namespace fast {
	...
	struct blobPoint {
		...
		inline void setBorderType(fast::border_type type) {
...
namespace hv::v1 {
	...
	private:
		...
		std::vector<std::shared_ptr<hv::v1::blobInfo>> _blobInfo;
	...
	public:
		...
		void compute(std::shared_ptr<hv::v1::image> image) {
			...
			if (image->pixel_type() != hv::v1::image_data_type::u8c1_image) {
				HVERROR(error, "Invalid image data type");
			}

			try {
				int rows = image->height();
				int cols = image->width();
				unsigned char* buffer = (unsigned char*)image->ptr();
				...
				for (auto& contour : contours) {
					...
					std::vector<std::shared_ptr<hv::v1::point>> features;
					for (auto& point : contour) {
						...
						std::shared_ptr<hv::v1::point> calPoint(new hv::v1::point("contour points", point.col, point.row));

						/*hv::v1::calPoint calPoint;
					...
					//흑백 구분
					bool isBlack = contour[0].border_type == fast::border_type::HOLE_BORDER ? true : false;
					if (isBlack == true) continue;


					std::shared_ptr<hv::v1::blobInfo> info(new hv::v1::blobInfo("info", id, min_x, min_y, width, height, centeroid_x, centeroid_y, isBlack));
		...
		inline void followBorder(std::vector<std::vector<int>>& image, int row, int col, fast::blobPoint p2, fast::border NBD, std::vector<std::vector<fast::blobPoint>>& contours) {
			...
			do {
				...
				if (current.samePoint(p2)) {
					...
					start.setBorderType(NBD.border_type);
			...
			while (true) {
				...
				p3.setBorderType(NBD.border_type);
...
hv::v1::blobDetector::blobDetector() : impl(new impl_fastBlob()) {

}
hv::v1::blobDetector::~blobDetector() {

}

void hv::v1::blobDetector::compute(std::shared_ptr<hv::v1::image> image) {

	if (image == nullptr) {
		HVERROR(error, "Invalid image info");
	}


	try {

		this->impl->compute(image);
	}
	catch (std::exception e) {
		throw e;
	}
}
std::vector<std::shared_ptr<hv::v1::blobInfo>>  hv::v1::blobDetector::blobInfo() {
	return this->impl->blobInfo();
}


std::vector<std::shared_ptr<hv::v1::blobInfo>> hv::v1::blobDetector::rectFilter(std::vector<std::shared_ptr<hv::v1::blobInfo>> blobInfo, int minWidth, int maxWidth, int minHeight, int maxHeight) {
	...
	std::vector<std::shared_ptr<hv::v1::blobInfo>> result;
...
void hv::v1::blobDetector::blobFillAll(std::shared_ptr<hv::v1::image> image, unsigned char pixel, std::vector<std::shared_ptr<hv::v1::blobInfo>> infos) {
	...
	for (auto info : infos) {
		try {
			hv::v1::blobDetector::blobFill(image, pixel, info);
		}
		catch (std::exception e) {
			throw e;
		}
...
void hv::v1::blobDetector::blobFill(std::shared_ptr<hv::v1::image> image, unsigned char pixel, std::shared_ptr<hv::v1::blobInfo> info) {
	...
	if (info->rectX() < 0 || info->rectX() > (int)image->width() || info->rectY() < 0 || info->rectY() > (int)image->height()) {
		HVERROR(error, "Invalid blob info");
	}

	if (info->rectWidth() + info->rectX() > (int)image->width() || info->rectHeight() + info->rectY() > (int)image->height()) {
		HVERROR(error, "Invalid blob info");
	}

	if (image->pixel_type() != hv::v1::image_data_type::u8c1_image) {
		HVERROR(error, "Invalid image channel");
	}
	...
	std::deque<hv::v1::calPoint> stack;
	for (int x = 0; x < blobWidth; x++) {
		hv::v1::calPoint point1;
		hv::v1::calPoint point2;
	...
	for (int y = 0; y < blobHeight; y++) {
		for (int x = 0; x < blobWidth; x++) {
			...
			int buffer_index = imageWidth * (y + blobStartY) + (x + blobStartX);
